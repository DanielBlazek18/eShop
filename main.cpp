...
/**@info eShop v.1.1 made from 95%, few features missing.
         More precisely, there is a lack feature ,,editace produktu". Number 6 in main menu.
*/
...
shop *funkce_pro_setrizeni_produktu_dle_id(shop *pole_produktu, int &velikost_pole) //Podpurna setrizovaci funkce (dle id o nejmensi po nejvetsi)
{
    /** @brief Podpurna setrizovaci funkce, ktera setridi zbozi dle id od nejmensi po nejvetsi,
        funkce je pouzita pro dalsi funkce:
        vzpis_do_konzole & vypis_do_csv.
        @param pole_produktu je promenna struktur, ktere obsahuje produkty.
        @param velikost_pole je cislo, ktere udava pocet produktu.
        @return vraci strukturu: pole_produktu_setrizeni
    */

    //Buble sort setrizeni podle id
    ...
    for(int i=0; i<velikost_pole; i++) //serazeni produktu podle id od nejmensi po nejvetsi
    {
        for(int j=0; j<i; j++)
        {
            if(pole_produktu_setrizeni[i].id < pole_produktu_setrizeni[j].id)
            {
                long int temp_cena = pole_produktu_setrizeni[i].cena;
                pole_produktu_setrizeni[i].cena = pole_produktu_setrizeni[j].cena;
                pole_produktu_setrizeni[j].cena = temp_cena;
...
void vypis_do_csv(shop *pole_produktu, int velikost_pole) //Jednoduchy vypis produktu do csv souboru
{
    /** @brief Funkce pro jednoduchy vypis produktu do csv souboru, vzdy setrizeny, dle id od nejmensiho po nejvetsi.
        @param pole_produktu je promenna struktur, ktere obsahuje produkty.
        @param velikost_pole je cislo, ktere udava pocet produktu.
    */
    ...
    cout << "+---------------------------------------------------------+\n";
    cout.width(69);
    cout << "|              Vypis produktu do csv souboru              |\n";
    cout.width(70);
    cout << "+---------------------------------------------------------+\n\n";
    ...
    if(velikost_pole == NULL) //Overeni zda jsou nacteny produkty
    {
        cout << "Seznam produktu je prazdny.\n";
    }
    else
    {
        cout << "Zadejte nazev pro vystupni soubor bez pripony csv: ";
        cin >> file_out;
        ...
        while(file_out.c_str()[o]!='\0') //Osetreni nazvy vystupniho souboru, v pripade chyby, volani funkci a navraceni zpet
        {
            if(!cin.good() || file_out.c_str()[o]=='.' || file_out.c_str()[o]=='/' || file_out.c_str()[o]=='\\'  || file_out.c_str()[o]==':' || file_out.c_str()[o]=='*' || file_out.c_str()[o]=='?' || file_out.c_str()[o]=='"' || file_out.c_str()[o]=='<' || file_out.c_str()[o]=='>' || file_out.c_str()[o]=='|')
            {
                chyba();
                chyba_nepovolene_znaky();
                return vypis_do_csv(pole_produktu, velikost_pole);
            }
            o++;
        }

        file_out = file_out + pripona; //Spojeni nazvu souboru a pripony

        funkce_pro_setrizeni_produktu_dle_id(pole_produktu, velikost_pole); //Funkce pro setrizeni produktu dle id

        ofstream vystup_soubor; //Vystupni proud do souboru
        vystup_soubor.open(file_out.c_str(), ios::out);
        ...
        else
        {
            for(int i=0; i<velikost_pole; i++)
            {
                vystup_soubor << pole_produktu[i].id << ";"
                              << pole_produktu[i].nazev << ";"
                              << pole_produktu[i].popis << ";"
                              << pole_produktu[i].cena << ";"
                              << pole_produktu[i].pocet_kusu << endl;
            }
            vystup_soubor.close();
            cout << "Ulozeni do souboru CSV probehlo uspesne.\n";
        }
    }
}
...
{
    /** @brief Funkce pro jednoduchy vypis produktu do konzole, vzdy setrizeny, dle id od nejmensiho po nejvetsi.
        @param pole_produktu je promenna struktur, ktere obsahuje produkty.
        @param velikost_pole je cislo, ktere udava pocet produktu.
    */
...
shop *pridani_produktu(shop *pole_produktu, int &velikost_pole) //funkce, ktera prida produkty do struktury
{
    clear_screen();
    cout.width(71);
    cout << "+--------------------------------------------------------+\n";
    cout.width(71);
    cout << "|                   Pridani produktu                     |\n";
    cout.width(72);
    cout << "+--------------------------------------------------------+\n\n";
...
shop *smazani_produktu(shop *pole_produktu, int &velikost_pole)
{

}

shop *editace_produktu(shop *pole_produktu, int &velikost_pole) //Funkce pro upravovani produktu - menu
{
    /** @brief Funkce pro zakladni upravu produktu, pridavani - mazani. Jedna se hlavne o hlavni menu.
        @param pole_produktu je promenna struktur, ktere obsahuje produkty.
        @param velikost_pole je cislo, ktere udava pocet produktu.
    */
    ...
    while(1)
    {
        ...
        cout << "[- Menu pro editaci produktu -]\n"
             << "* 1. Pridat produkt\n"
             << "* 2. Odstranit produkt\n"
             << "* 3. Navrat do hlavniho menu\n";
        ...
        switch(volba)
        {
            case 1: pole_produktu = pridani_produktu(pole_produktu, velikost_pole);
                    break;

            case 2: pole_produktu = smazani_produktu(pole_produktu, velikost_pole);
                    break;

            case 3: return pole_produktu;
                    return 0;
                    break;
...
int main() //Hlavni cast programu, obsahujici hlavni menu a prepinac switch pro hlavni menu
{
    ...
    */
    ...
    while(1)
    {
        ...
        if(velikost_pole != NULL) //Informace pro uzivatele, zda je nacten vstupni soubor
        {
            cout << " [OK] \n";
        }
        else cout << " [-] \n";

        cout << "* 2. Vypis produktu do konzole\n"
             ...
             << "* 6. Editace produktu\n"
             << "* 7. Vypis produktu do csv souboru\n"
             << "* 8. Autor programu\n"
             << "* 9. Konec programu\n";
            ...
            switch(volba)
            {
                ...
                case 6: pole_produktu = editace_produktu(pole_produktu, velikost_pole);
                        break;

                case 7: vypis_do_csv(pole_produktu, velikost_pole);
                        break;

                case 8: autor();
                        break;

                case 9: konec_programu();
                        free(pole_produktu);
                        return 0;
